...
app.post("/restaurantes", async (req, res) => {
  try {
    ...
    res.status(201).json({msg: "Restaurante criado com sucesso: ", novoRestaurante});
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
});
...
app.get("/restaurantes", async (req, res) => {
  try {
    ...
    res.status(200).json({msg: "Todos os restaurantes: ", restaurantes});
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
app.put("/restaurantes/:id", upload.single('foto') , async (req, res) => {
  try {
    const { id } = req.params;
    const atualizacoes = req.body;
    if(req.file){
      atualizacoes.foto = `uploads/${req.file.filename}`;
    }
...
app.delete("/restaurantes/:id", async (req, res) => {
  try {
    const restauranteDeletado = await Restaurante.findByIdAndDelete(
      req.params.id
    );
...
app.post("/restaurantes/registro", upload.single('foto') , async (req, res) => {
  try {
    console.log(req.body)
    let foto = req.body.foto
    let endereco = JSON.parse(req.body.endereco)
    const { nome, email, senha, telefone, categoria } = req.body;
    if(req.file){
